...
public static class MetricsExporterExtensions
{
    ...
    public static string CreateSummary(this MetricsExporter exporter, PipelineMetricsSnapshot snapshot)
    {
        ...
        foreach (var policy in snapshot.PolicySnapshots.OrderByDescending(p => p.SuccessRate))
        {
            ...
            sb.AppendLine($" Failures: {policy.FailedExecutions:N0} | State: {state}");
        }
    ...
    public static string ExportConsoleTable(this MetricsExporter exporter, PipelineMetricsSnapshot snapshot, bool includePolicyDetails = true)
    {
        ...
        // Header
        sb.AppendLine("╔════════════════════════════════════════════════════════════════╗");
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-63} ║", "Resilience Pipeline Metrics"));
        sb.AppendLine("╠════════════════════════════════════════════════════════════════╣");
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-20} {1,-42}║", "Exported at:", DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture)));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-20} {1,-25:N0}{2,18}║", "Total executions:", snapshot.TotalExecutions, ""));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-20} {1,-25:N0}{2,18}║", "Successful executions:", snapshot.SuccessfulExecutions, ""));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-20} {1,-25:N0}{2,18}║", "Failed executions:", snapshot.FailedExecutions, ""));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-20} {1,-25}{2,18}║", "Success rate:", snapshot.SuccessRate.ToString("P2", CultureInfo.InvariantCulture), ""));
        sb.AppendLine("╠════════════════════════════════════════════════════════════════╣");

        if (includePolicyDetails)
        {
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-30} {1,-20} {2,-12} {3,-12} {4,-8}║", "Policy Name", "Type", "Executions", "Success", "Rate"));
            sb.AppendLine("╠════════════════════════════════════════════════════════════════╣");
            ...
            foreach (var policy in snapshot.PolicySnapshots.OrderByDescending(p => p.TotalExecutions))
            {
                ...
                sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-30} {1,-20} {2,-12:N0} {3,-12:N0} {4,-8:P1}║",
                    policy.PolicyName, policy.PolicyType, policy.TotalExecutions, policy.SuccessfulExecutions, policy.SuccessRate));
                sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "║ {0,-30} {1,-20} {2,-12} {3,-12:N0} {4,-8} State: {5,-8}║",
    ...
    public static string ExportMarkdown(this MetricsExporter exporter, PipelineMetricsSnapshot snapshot)
    {
        ...
        foreach (var policy in snapshot.PolicySnapshots.OrderByDescending(p => p.SuccessRate))
        {
            ...
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture, "| {0} | {1} | {2:N0} | {3:N0} | {4:N0} | {5:P1} | {6} |",
